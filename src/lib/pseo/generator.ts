...
function attachLinks(pages: DraftPage[]): DraftPage[] {
  return pages
    .map((page) => {
      ...
      const structuralParent = pages
        .filter((candidate) => {
          const candidatePathname = pathnameFromUrl(candidate.url);
          return (
            candidate.url !== page.url &&
            candidatePathname !== "/" &&
            pagePathname.startsWith(`${candidatePathname}/`)
          );
        })
        .sort(
          (left, right) =>
            pathnameFromUrl(right.url).length - pathnameFromUrl(left.url).length,
        )[0];

      const semanticParent = pages
        .filter(
          (candidate) =>
            candidate.url !== page.url &&
            candidate.playbook_type !== page.playbook_type &&
            (candidate.topic_key === page.topic_key ||
              sharesAny(candidate.entity_keys, page.entity_keys)),
        )
        .sort((left, right) => {
          const leftCategoryBoost =
            left.playbook_type === "directory" ||
            left.playbook_type === "curation" ||
            left.playbook_type === "glossary"
              ? -1
              : 0;
          const rightCategoryBoost =
            right.playbook_type === "directory" ||
            right.playbook_type === "curation" ||
            right.playbook_type === "glossary"
              ? -1
              : 0;
          const leftShared = Number(
            left.topic_key === page.topic_key || sharesAny(left.entity_keys, page.entity_keys),
          );
          const rightShared = Number(
            right.topic_key === page.topic_key || sharesAny(right.entity_keys, page.entity_keys),
          );

          return (
            leftCategoryBoost - rightCategoryBoost ||
            rightShared - leftShared ||
            left.url.length - right.url.length
          );
        })[0];
      const parent = structuralParent || semanticParent;

      const samePlaybookSiblings = pages
        .filter(
          (candidate) =>
            candidate.url !== page.url &&
            candidate.playbook_type === page.playbook_type,
        ...
        .sort((left, right) => {
          ...
          return rightShared - leftShared || left.url.localeCompare(right.url);
        })
        .slice(0, 2);

      const supplementalSiblings = pages
        .filter(
          (candidate) =>
            candidate.url !== page.url &&
            candidate.url !== parent?.url &&
            candidate.playbook_type !== page.playbook_type &&
            !samePlaybookSiblings.some((sibling) => sibling.url === candidate.url) &&
            (candidate.topic_key === page.topic_key ||
              sharesAny(candidate.entity_keys, page.entity_keys)),
        )
        .sort((left, right) => {
        ...
        .slice(0, Math.max(0, 2 - samePlaybookSiblings.length));

      const siblings = [...samePlaybookSiblings, ...supplementalSiblings];

      const crossPlaybook = pages
        .filter(
          (candidate) =>
            candidate.url !== page.url &&
            candidate.playbook_type !== page.playbook_type &&
            candidate.url !== parent?.url &&
            !siblings.some((sibling) => sibling.url === candidate.url) &&
            (candidate.topic_key === page.topic_key ||
              sharesAny(candidate.entity_keys, page.entity_keys)),
        )
      ...
      if (!parent || siblings.length < 2 || crossPlaybook.length < 2) {
        return null;
      }
      ...
      const internalLinks = dedupeByUrl([
        {
          url: parent.url,
          anchor: parent.content.h1,
          relation: "parent" as const,
        },
        ...siblings.map((candidate) => ({
          url: candidate.url,
          anchor: candidate.content.h1,
          relation: "sibling" as const,
        })),
        ...crossPlaybook.map((candidate) => ({
          url: candidate.url,
          anchor: candidate.content.h1,
      ...
      const relatedPages = dedupeByUrl(
        [...crossPlaybook, ...siblings]
          .slice(0, 3)
          .map((candidate) => ({
            url: candidate.url,
            title: candidate.content.h1,
            reason:
              candidate.playbook_type === page.playbook_type
                ? `Sibling ${candidate.playbook_type} page that helps the reader compare adjacent options.`
                : `${candidate.playbook_type} page that expands the same topic from a different search intent.`,
          })),
      );
